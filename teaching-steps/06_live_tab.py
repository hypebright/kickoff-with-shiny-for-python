...
Run this with:
    shiny run teaching-steps/06_live_tab.py --reload --launch-browser

What's new since Step 5: the Live tab's two placeholder text outputs from
Step 3 are gone, replaced with matches from yesterday,
today, and tomorrow, with live scores where available.
"""
...
def server(input, output, session):
    ...
    # ── Everything below is new for this step ──────────────────────────

    def match_row(dt, game):
        # A single match (simple one line of HTML).
        ...
        # The API reports match progress as a free-text field (time_elapsed),
        # so could be anything: "notstarted", "45", "HT", "finished". For simplicity
        # we just make it two values (finished / live).
    ...
    def live():
        ...
        # date.today() is today's calendar date (without time). We
        # build yesterday and tomorrow from it with timedelta.
        today = date.today()
        yesterday = today - timedelta(days=1)
        tomorrow = today + timedelta(days=1)

        # Move every match into one of three lists based on its date.
        # dt.date() strips the time off a datetime, leaving just the date
        # part, so it can be compared directly against today/yesterday/
        # tomorrow.
